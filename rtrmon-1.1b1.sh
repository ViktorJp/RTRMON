...
# RTRMON v1.1b1 - Asus-Merlin Router Monitor
...
logo () {
  ...
  echo -e "   / _, _/ / / / _, _/ /  / / /_/ / /|  /   ${CRed}(N)${CGreen}ext Page ($NextPage/3)${CYellow}"
...
vconfig () {
  ...
  if [ -f $CFGPATH ]; then #Making sure file exists before proceeding
    ...
    while true; do
      ...
      echo -e "${InvDkGray}${CWhite} 7 ${CClear}${CCyan}: Temperature Units (C/F/K)     :"${CGreen}$TempUnits
      echo -en "${InvDkGray}${CWhite} 8 ${CClear}${CCyan}: Enable Ookla Speedtest? (Y/N) :"${CGreen}
      if [ "$Speedtst" == "0" ]; then
        printf "No"; printf "%s\n";
      ...
      # Execute chosen selections
          case "$ConfigSelection" in
            ...
            3) # -----------------------------------------------------------------------------------------
              ...
              MaxSpeedInetUL=$MaxSpeedInetUL1
            ;;
            ...
            8) # -----------------------------------------------------------------------------------------
              echo ""
              if [ -f $OOKLAPATH ]; then
                echo -e "${CCyan}7. Would you like to disable and uninstall the Ookla"
                ...
                echo -e "${CCyan}7. Would you like to install and enable the Ookla"
...
calculatestats () {
  ...
  # Disk - JFFS
    disk_use=$($timeoutcmd$timeoutsec df -P | grep /dev | grep -E '/jffs' | awk '{print $2, $3}')
...
DisplaySpdtst () {
  ...
  #run speedtest and save Results
    ...
    SpdDate=$(date)
    SpdServer=$(echo $speed | awk -F ',' 'NR==1 {print $1, $2}' | sed -e 's/^"//' -e 's/"$//')
    SpdLatency=$(echo $speed | awk -F ',' 'NR==1 {print $4}' | sed -e 's/^"//' -e 's/"$//')
    SpdJitter=$(echo $speed | awk -F ',' 'NR==1 {print $5}' | sed -e 's/^"//' -e 's/"$//')
    SpdPacketLoss=$(echo $speed | awk -F ',' 'NR==1 {print $6}' | sed -e 's/^"//' -e 's/"$//')
    SpdDownload=$(echo $speed | awk -F ',' 'NR==1 {print $7}' | sed -e 's/^"//' -e 's/"$//')
    SpdUpload=$(echo $speed | awk -F ',' 'NR==1 {print $8}' | sed -e 's/^"//' -e 's/"$//')
  ...
  echo ""
  preparebar 35 "|"
  progressbar $SpdDownload $MaxSpeedInet " DL vs WAN " "Mbps" "Reverse"
  echo ""
  preparebar 35 "|"
  progressbar $SpdUpload $MaxSpeedInetUL " UL vs WAN " "Mbps" "Reverse"
  echo ""
  echo ""
...
DisplayPage2 () {
  ...
  echo ""
  preparebar 35 "|"
  progressbar $oldwantxmbrate $MaxSpeedInetUL " Avg WAN TX" "Mbps" "Standard" $oldwantxmbratedisplay
  echo ""
  echo -e "${CGreen} ___"
  ...
  echo ""
  preparebar 35 "|"
  progressbar $oldlantxmbrate $MaxSpeedLAN " Avg LAN TX" "Mbps" "Standard" $oldlantxmbratedisplay
  echo ""
  echo -e "${CGreen} ____"
  ...
  if [ "$MaxSpeed24Ghz" != "0" ]; then
    preparebar 35 "|"
    progressbar $oldw24rxmbrate $MaxSpeed24Ghz " Avg 24G RX" "Mbps" "Standard" $oldw24rxmbratedisplay
    echo ""
    preparebar 35 "|"
    progressbar $oldw24txmbrate $MaxSpeed24Ghz " Avg 24G TX" "Mbps" "Standard" $oldw24txmbratedisplay
    echo ""
    preparebar 35 "|"
    ...
    echo -e "${InvCyan} ${CClear}${CCyan} wl0 Disabled"
    echo ""
  fi
  if [ "$MaxSpeed5Ghz" != "0" ]; then
    preparebar 35 "|"
    progressbar $oldw5rxmbrate $MaxSpeed5Ghz " Avg 50G RX" "Mbps" "Standard" $oldw5rxmbratedisplay
    echo ""
    preparebar 35 "|"
    progressbar $oldw5txmbrate $MaxSpeed5Ghz " Avg 50G TX" "Mbps" "Standard" $oldw5txmbratedisplay
    echo ""
    preparebar 35 "|"
    converttemps $oldw1temp
    progressbar $currenttemp $currentrange " 5.0G Temp " $TempUnits "Standard"
  else
    echo -e "${InvCyan} ${CClear}${CCyan} wl1 Disabled"
  fi
}

...
# RTRMON v1.33b2 - Asus-Merlin Router Monitor by Viktor Jaep, 2022
#
    ...
    while [ $vpn -ne 5 ]; do
      ...
      if [ $VPNState -eq 2 ]; then
        ...
        if [ "$lastvpnip" != "$vpnip" ]; then
          ...
          vpncity="$(eval $vpncity)"; if echo $vpncity | grep -qoE '\b(error.*:.*True.*|Undefined)\b'; then vpncity="Not Found"; fi
          echo -e "$(date) - RTRMON - API call made to determine location of $vpnip in $vpncity" >> $LOGFILE
        fi
        vpnon="True"
        break
      else
  ...
  while [ $vpn -ne 5 ]; do
    ...
    if [ $VPNState -eq 2 ]; then
      ...
      if [ "$lastvpnip" != "$oldvpnip" ]; then
        ...
        oldvpncity="$(eval $oldvpncity)"; if echo $oldvpncity | grep -qoE '\b(error.*:.*True.*|Undefined)\b'; then oldvpncity="Not Found"; fi
        vpncity=$oldvpncity

...
# RTRMON v1.33b3 - Asus-Merlin Router Monitor by Viktor Jaep, 2022
#
  ...
  # Network - WAN/LAN/DNS IP Addresses
    ...
    while [ $vpn -ne 5 ]; do
      ...
      if [ $VPNState -eq 2 ]; then
        TUN="tun1"$vpn
        NVRAMVPNIP=$($timeoutcmd$timeoutsec nvram get vpn_client"$vpn"_addr)
        ...
        if [ "$(echo $NVRAMVPNIP | grep -E '^(192\.168|10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.)')" ]; then
          ...
          if [ -z $vpnip ]; then vpnip=$lastvpnip; fi
  ...
  while [ $vpn -ne 5 ]; do
    ...
    if [ $VPNState -eq 2 ]; then
      ...
      if [ "$(echo $NVRAMVPNIP | grep -E '^(192\.168|10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.)')" ]; then
        ...
        if [ -z $oldvpnip ]; then oldvpnip=$lastvpnip; fi

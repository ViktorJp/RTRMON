...
# RTRMON v1.19 - Asus-Merlin Router Monitor by Viktor Jaep, 2022
#
...
vconfig () {
  ...
  if [ -f $CFGPATH ]; then #Making sure file exists before proceeding
    ...
    while true; do
      ...
      echo -e "${InvDkGray}${CWhite} 9 ${CClear}${CCyan}: WAN0 Interface Override?      :"${CGreen}$WANOverride
      ...
      # Execute chosen selections
          case "$ConfigSelection" in
            ...
            7) # -----------------------------------------------------------------------------------------
              echo ""
              echo -e "${CCyan}7. Which Temperature Units would you prefer? (C)elcius,"
            ...
            8) # -----------------------------------------------------------------------------------------
              echo ""
              if [ -f $OOKLAPATH ]; then
                echo -e "${CCyan}8. Would you like to disable and uninstall the Ookla"
                ...
                echo -e "${CCyan}8. Would you like to install and enable the Ookla"
            ...
            9) # -----------------------------------------------------------------------------------------
              validinterfaces=$(ip -br link | LC_ALL=C awk '$2 == "UP" {print $1}')
              echo ""
              echo -e "${CCyan}9. Would you like to override the default interface assigned${CClear}"
              ...
              echo -e "${CCyan}to override should you be using something else. NOTE: The${CClear}"
              echo -e "${CCyan}interface name must be spelled correctly as listed below and${CClear}"
              echo -e "${CCyan}is case-sentive."
              echo ""
              echo -e "${CGreen}Valid interface choices:"
              echo -e "${CGreen}Auto"
              echo -e "$validinterfaces"
              echo ""
              echo -e "${CYellow}(Default = Auto, or choose one of the above)${CClear}"
              read -p 'Interface Override: ' WANOverride1
              if [ -z "$WANOverride1" ]; then WANOverride="Auto"; else WANOverride=$WANOverride1; fi
              if [ "$WANOverride" = "auto" ]; then WANOverride="Auto"; fi
            ;;
...
DisplayPage2 () {
  ...
  if [ "$MaxSpeed5Ghz" != "0" ]; then
    preparebar 35 "|"
    progressbar $oldw5rxmbrate $MaxSpeed5Ghz " Avg 50G RX" "Mbps" "Standard" $oldw5rxmbratedisplay $MaxSpeed5Ghz
    echo ""
    preparebar 35 "|"
    progressbar $oldw5txmbrate $MaxSpeed5Ghz " Avg 50G TX" "Mbps" "Standard" $oldw5txmbratedisplay $MaxSpeed5Ghz
    echo ""
    preparebar 35 "|"
    converttemps $oldw1temp
    progressbar $currenttemp $currentrange " 5.0G Temp " $TempUnits "Standard" $currenttemp $currentrange
  else
